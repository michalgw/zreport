...
begin
  Result:= '';
  i:= 1;
  while i<=CodePointLength(Value) do
    begin
      if (Value[i] = '#') or (Value[i] = Quote) then
        begin
          ps:= CodePointPos(Quote, CodePointCopy(Value, i+1, CodePointLength(Value)));
          pc:= Pos('#' , CodePointCopy(Value, i+1, CodePointLength(Value)));
          if (pc = 0) or (ps > 0) and (ps < pc) then pc:= ps;
          if pc = 0 then pc:= CodePointLength(Value);
          if Value[i] = '#' then
            Result:= Result + Char(StrToInt(CodePointCopy(Value, i+1, pc-1)))
          else
            begin
              Result:= Result + CodePointCopy(Value, i+1, pc-1);
              inc(i);
            end;
          Inc(i, pc);
        end
...
begin
  Result:= (Pos < CodePointLength(S)) and (CodePointCopy(S, Pos, 1) = EscapeChar) and CharIsCodeMap[CodePointCopy(S, Pos+1, 1)[1]]
end;
...
begin
  Result:= 0;
  i     := 1;
  while i <= CodePointLength(S) do begin
...
function EscapeLength(const S: String): Integer;
begin
  Result := CodePointLength(S) - 2 * EscapeCount(S);
end;
...
begin
  ...
  i:= 1;
  p:= EscapePos(S, Pos);
  while (i < CodePointLength(S)) and (i < p) do begin
    if CharIsCode(S, i) then begin
        Inc(i);
        C := CharToCodeMap[CodePointCopy(S, i, 1)[1]];
...
begin
  Result:= S;
  i     := 1;
  while i <= CodePointLength(Result) do
    if CharIsCode(Result, i) then
      UTF8Delete(Result, i, 2)
    else
      Inc(i);
end;
...
begin
  ...
  while i < CodePointLength(Result) do
    if CharIsCode(Result, i) then begin
      C := CharToCodeMap[CodePointCopy(Result, i+1, 1)[1]];
      if CodeIsStyle(C, E, O) then begin
        if (E in Styles) then
          if O then
            UTF8Delete(Result, i, 2)
...
begin
  ...
  if Index > EscLen then
    ...
    Result := {EscapeNormalize}(
                EscapePrefix(StartC, []) +
                CodePointCopy(S, StartI, EndI-StartI+1) +
                EscapeSuffix(EndS) );
  end;
end;
...
begin
  ...
  Finish := EscapeCopy(Result, Index+Count, CodePointLength(Result));
...
begin
  ...
  Finish := EscapeCopy(Result, Pos, CodePointLength(Result));
...
begin
  if EOL then
    ...
    l  := CodePointLength(fLine);
    i  := 0;
    while (si+i <= l) and not CharIsCode(fLine, si+i) do Inc(i);
    fToken  := CodePointCopy(fLine, si, i);
    fFinish := fStart + i;
  end;
  Result := fToken;
end;
...
var
  ...
  es: TZREscapeStyle;
initialization
  EscapeModelNames[emCustom] := szrEscapeModelCustom;
  EscapeModelNames[emNone]   := szrEscapeModelNone;
