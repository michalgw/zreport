...
type
  ...
  public
    ...
    destructor  Destroy; override;
    property OperatorType: TZROperator read fOperator;
  end;
...
begin
  if fExpression <> Value then begin
    ...
    fErrorMsg   := Format(szrExpressionParse, [Expression]);
  end;
end;
...
begin
  if not Assigned(fElement) then
    ...
    on E: Exception do begin
      fErrorMsg:= Format(szrCalculationError, [Expression, E.Message]);
      Result   := fErrorMsg;
    end;
  end;
end;
...
function TZREvaluator.Parse: Boolean;
  ...
  procedure SetError(const ID: String; const Args: array of const);
  begin
    fErrorMsg:= Format(ID, Args);
  end;
    ...
    begin
      ...
      if (Left <> nil) and FindOperator then begin
        ...
        if Right <> nil then begin
          Result:= TZRElementOperator.Create(OperatorType, Left, Right);
...
procedure TZRElementFunction.SetValue(AValue: Variant);
begin
  if (fLibrary <> nil) and (fIndex >= 0) and (fIndex < fLibrary.Count) then
    fLibrary.SetValue(fIndex, AValue);
end;
