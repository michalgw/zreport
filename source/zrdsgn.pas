...
type
  ...
  private
    ...
    function CompareNodes(lParam1, lParam2: TTreeNode): Integer;
  public
    { Public declarations }
...
begin
  None             := szrDesignNone;
  btClose .Caption := szrDesignClose;

  tsBands .Caption := szrDesignTSBands;
  tsVars  .Caption := szrDesignTSVars;

  gbBands .Caption := szrDesignGBBands;
  gbGroups.Caption := szrDesignGBGroups;
  tvBands .Hint    := szrDesignTVBands;
  lbGroups.Hint    := szrDesignLBGroups;

  acBandsNew    .Caption := szrDesignACBandsNew;
  acBandsDelete .Caption := szrDesignACBandsDelete;
  acBandsRename .Caption := szrDesignACBandsRename;
  acGroupsNew   .Caption := szrDesignACGroupsNew;
  acGroupsDelete.Caption := szrDesignACGroupsDelete;

  sbGroupsUp  .Caption := '';
  sbGroupsDown.Caption := '';

  gbData       .Caption     := szrDesignGBData;
  grControllers.Hint        := szrDesignGRControllers;
  grControllers.Cells[0, 0] := szrDesignCellController;
  grControllers.Cells[1, 0] := szrDesignCellDataSet;

  laDataFields    .Caption  := szrDesignLADataFields;
  lbDataFields    .Hint     := szrDesignLBDataFields;

  gbVariables      .Caption     := szrDesignGBVariables;
  laVariableFields.Caption      := szrDesignLAVariableFields;
  lbVariableFields.Hint         := szrDesignLBVariableFields;
  laVariableExpressions.Caption := szrDesignLAVariableExpressions;
  lbVariableExpressions.Hint    := szrDesignLBVariableExpressions;
  laVariableTotals.Caption      := szrDesignLAVariableTotals;
  lbVariableTotals.Hint         := szrDesignLBVariableTotals;

  acVariablesField     .Caption := szrDesignACVariablesField;
  acVariablesExpression.Caption := szrDesignACVariablesExpression;
  acVariablesTotal     .Caption := szrDesignACVariablesTotal;
  acVariablesDelete    .Caption := szrDesignACVariablesDelete;
end;
...
begin
  ...
  for i:= 0 to tvBands.Items.Count-1 do
    if tvBands.Items[i].Data = Pointer(Band) then begin
...
begin
  ...
  if (Band is TZRCustomController) then
    for i:= 0 to TZRCustomController(Band).GroupList.Count-1 do begin
      Group := TZRGroup(TZRCustomController(Band).GroupList[i]);
...
function TZRDesignForm.CompareNodes(lParam1, lParam2: TTreeNode): Integer; // stdcall;
...
begin
  ...
  if (pcDesign.ActivePage = tsBands) and (lbGroups.ItemIndex >= 0) then
    Designer.SelectOnlyThisComponent(TZRGroup(lbGroups.Items.Objects[lbGroups.ItemIndex]));
end;
...
begin
  ...
  Designer.SelectOnlyThisComponent(TZRCustomBand(tvBands.Selected.Data));
end;
...
begin
  ...
  Designer.SelectOnlyThisComponent(TZRCustomBand(grControllers.Objects[0, grControllers.Selection.Top]));
end;
...
begin
  ...
  if (pcDesign.ActivePage = tsVars) and (i >= 0) and (i < B.Items.Count) then
    Designer.SelectOnlyThisComponent(TZRVariable(B.Items.Objects[i]));
  acVariablesUpdate;
end;
...
begin
  ...
  if Accept then begin
    ChildBand  := TZRCustomBand((Source as TTreeView).Selected.Data);
    ParentBand := TZRCustomBand((Sender as TTreeView).DropTarget.Data);
