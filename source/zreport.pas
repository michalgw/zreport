...
type
  ...
  private
    ...
    procedure SetMaster(AValue : TZRCustomController); override;
  protected
    function EvaluateValue : Variant; override;
  ...
  private
    ...
    procedure SetVariable(AValue : TZRVariable);
  protected
    function  EvaluateValue : Variant; override;
    procedure ResetValue; override;
  public
    procedure ResetValues(Index: Integer);
    procedure UpdateValues(Index: Integer; AValue: Variant);
  ...
  public
    ...
    function GetBand(ABandType: TZRBandType): TZRCustomBand;
    function CreateBand(ABandType : TZRBandType; AGroup: TZRGroup): TZRCustomBand;
    procedure AddBand   (ABand: TZRCustomBand);
    procedure RemoveBand(ABand: TZRCustomBand);

    function CreateGroup: TZRGroup;
    procedure AddGroup   (AGroup: TZRGroup);
    procedure RemoveGroup(AGroup: TZRGroup);
...
begin
  ...
  //fMinMaxButton.SetBounds(0, 0, 16{GetSystemMetrics(SM_CXSMICON)}, 16{GetSystemMetrics(SM_CYSMICON)});
...
begin
  if (csDesigning in ComponentState) and Assigned(DesignForm) then
    DesignForm.WindowProc(TLMessage(Message));
end;
  ...
  begin
    if Assigned(Margins) then with Canvas do begin
      ...
      Rectangle(
        ...
        (Self.Width -Margins.Right ) * CharSize.X + 2,
        (Self.Height-Margins.Bottom) * CharSize.Y + 2);
    end;
  end;
