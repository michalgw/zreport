...
uses SysUtils, Classes, Controls, Forms, Graphics, Dialogs, StdCtrls, Buttons,
  ExtCtrls, ComCtrls, ZRPrntr, ZRSearch, RichMemo, LCLType;
...
begin
  Preview.Lines.Clear;
  Status .Caption:= szrProcessing;
  Exit   .Caption:= szrExit;
  Print  .Caption:= szrPrintReport;
  Search .Caption:= szrSearchReport;
  FirstPage .Hint:= szrFirstPageHint;
  PrevPage  .Hint:= szrPrevPageHint;
  NextPage  .Hint:= szrNextPageHint;
  LastPage  .Hint:= szrLastPageHint;
  PageNo    .Hint:= szrPageNoHint;
  Setup     .Hint:= szrPrinterSetup;
  Save      .Hint:= szrSaveReportHint;
  Load      .Hint:= szrLoadReportHint;
  Print     .Hint:= szrPrintReportHint;
  Search    .Hint:= szrSearchHint;
end;
...
procedure TZRPreviewForm.FormClose(Sender: TObject; var AAction: TCloseAction);
begin
  AAction := caFree;
end;
...
begin
  if Printer <> Value then begin
    ...
    if Assigned(Printer) then begin
      ...
      if Preview.Font.Size = 0 then
        Preview.Font.Size := GetDefaultFontSize;
...
var
  ...
  begin
    ...
    while not T.EOL do begin
      ...
      Preview.GetTextAttributes(p, FS);
      FS.Style := Y;
      Preview.SetTextAttributes(p, l, FS);
      Inc(p, l);
      T.NextToken;
    end;
  end;
...
var
  S : String;
begin
  S := InputBox(szrGotoPageNo, '', IntToStr(PageNumber));
...
*)
  ...
  begin
    ...
    if Printer.Found then
      ...
      begin
        ShowMessageFmt(szrSearchNotFound,[s]);
      end;
    fSearch := s;
  end;
