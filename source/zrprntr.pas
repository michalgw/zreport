...
const
  CM_ZRPROGRESSUPDATE = LM_USER + 0;
  CM_ZRPAGEFINISHED   = LM_USER + 1;
...
type
  ...
  public
    ...
    function EndOfPage(APreview: Boolean) : String;
...
begin
  ...
  if fHandle = 0 then begin
    ...
    fPrinter.RawMode := True;
    fPrinter.BeginDoc;
    fHandle := PtrUInt(fPrinter);
  end;
end;
...
begin
  ...
  fPrinter.Write(Buffer, Count, Bytes);
  Result:= Bytes;
end;
...
const
  MessageMap: array[TZRPrinterStatus] of PString = (
    @szrPrinterNotReady, @szrPrinterNotBusy, @szrPrinterNotFinished, nil );
begin
...
function TZRPrinter.EndOfPage(APreview: Boolean): String;
begin
  case Options.PaperType of
    zptFolio : Result := Scissors;
    zptSheet : if APreview then Result := '' else Result := EscapeFormat('', [ecFormFeed], []);
    else       Result := '';
  end;
end;
...
begin
  ...
  AEndPage   := Options.PageTo;

  if (StartPage > AEndPage  ) or
     (StartPage < 0        ) or
     (AEndPage   > PageCount) then ZRError(@szrIllegalPageRange, [StartPage, AEndPage]);

  if AEndPage < StartPage then begin
    i := AEndPage;  AEndPage := StartPage;  StartPage := i;
  end;
  if StartPage < 1         then StartPage := 1;
  if AEndPage   > PageCount then AEndPage   := PageCount;
  ...
  for i := FFoundPage to pagecount-1 do
    begin
      ...
      for j := fr to Report.GetPageHeight-1 do
        begin
          fline := EscapeDeformat(spage.Line[j]);
          if (i = fFoundPage) and (j = fFoundLine) then
            begin
              fline := copy(fline,FoundPos,length(fline));
            end;
            if fIgnoreCase then
              fline := AnsiUpperCase(fline);
            ps   := AnsiPos(toFind,fline);
            if ps  <> 0 then
              begin
